...
def nueva_resolucion(img, nueva_resolucion=None):
    ...
    img = img.resize(nueva_resolucion) # Ingresa código aquí
...
def detecta_bordes(img):
    ...
    if num_canales == 1:
        ...
        bordes = cv2.Canny(cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY), 100, 200) # Ingresa código aquí
...
def normaliza(img):
    ...
    arreglo = arreglo / 255.0 # Ingresa código aquí
...
def transformar_color(img, espacio='HSV'):
    ...
    if espacio == 'HSV':
        ...
        img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2LAB) # Ingresa código aquí
    elif espacio == 'YCrCb':
        img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2YCrCb) # Ingresa código aquí
    else:
...
def aplicar_filtro(img, tipo_filtro="GaussianBlur", **kwargs):
    ...
    if tipo_filtro == "GaussianBlur":
        ...
        filtrada = cv2.Laplacian(img_np, cv2.CV_64F) # Ingresa código aquí
...
def binariza(arreglo_img, umbral=128):
    ...
    """
    segmentada = (arreglo_img > umbral).astype(np.uint8) # Ingresa código aquí
    return segmentada
...
def aplicar_mascara(img_array, mascara):
    ...
    img_mascarada = img_array * mascara # Ingresa código aquí
...
def aplicar_filtro_convolucion(img_array, kernel):
    ...
    for i in range(img_array.shape[0]): # Ingresa código aquí
        for j in range(img_array.shape[1]): # Ingresa código aquí
